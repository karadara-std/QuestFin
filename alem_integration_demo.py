...
def analyze_with_alemllm(financial_data):
    ...
    # Подготавливаем сообщение для ИИ
    user_message = f"""
    Проанализируй финансовую ситуацию пользователя и дай 3 конкретных совета:
    ...
    payload = {
        "model": "alem-llm-latest",  # Модель Alem AI
        "messages": [
            {
                "role": "system",
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/chat/completions",  # Эндпоинт AlemLLM
            headers=HEADERS,
            json=payload,
            timeout=10  # Ждём максимум 10 секунд
        )
        ...
        if response.status_code == 200:
            ...
            # Если API вернул ошибку — показываем демо-ответ
...
def get_financial_score(financial_data):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/score",  # Эндпоинт Score API
            headers=HEADERS,
            json=score_payload,
            timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            # Демо-расчёт если API недоступен
...
def generate_character_art(character_level, score):
    ...
    Промпт меняется в зависимости от уровня пользователя.
    """
    print("\n🎨 [3/3] Вызов Text-to-Image для генерации персонажа...")
    
    # Разные образы персонажа в зависимости от уровня финансовой грамотности
    character_prompts = {
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/images/generate",  # Эндпоинт Text-to-Image
            headers=HEADERS,
            json=image_payload,
            timeout=30  # Генерация изображений занимает дольше
        )
        ...
        if response.status_code == 200:
            ...
            print(f"   ⚠️  API вернул {response.status_code}. В реальном приложении — запасной арт.")
            return {
                "success": False,
                "image_url": f"https://placeholder.com/character_level_{character_level}.png",
                "note": "Демо: реальная генерация требует активного ключа Alem AI"
            }
...
def calculate_xp_reward(score, previous_score=65):
    """
    Внутренняя логика FinQuest:
    Рассчитываем XP-награду на основе скора и прогресса.
    """
    base_xp = 50  # Базовая XP за месяц
    ...
    total_xp = base_xp + improvement_bonus + tier_bonus
    return total_xp


def run_finquest_demo():
    """
    Главная функция — демонстрирует полный цикл FinQuest
    """
    print("=" * 60)
    print("🎮 FinQuest — Демонстрация интеграции Alem AI")
    print("=" * 60)
    print(f"\n👤 Пользователь: {USER_FINANCIAL_DATA['user_id']}")
    print(f"📅 Период: {USER_FINANCIAL_DATA['month']}")
    print(f"⚔️  Персонаж: Уровень {USER_FINANCIAL_DATA['character_level']}, {USER_FINANCIAL_DATA['character_xp']} XP")
    
    # === ШАГ 1: Получить совет от ИИ ===
    llm_result = analyze_with_alemllm(USER_FINANCIAL_DATA)
    
    # === ШАГ 2: Получить финансовый скор ===
    score_result = get_financial_score(USER_FINANCIAL_DATA)
    current_score = score_result["score"]
    
    # === ШАГ 3: Рассчитать XP и проверить повышение уровня ===
    ...
    # === ШАГ 4: Если новый уровень — генерируем новый арт ===
    character_result = None
    if level_up:
        print(f"\n🎉 ПОВЫШЕНИЕ УРОВНЯ! {USER_FINANCIAL_DATA['character_level']} → {new_level}")
        character_result = generate_character_art(new_level, current_score)
    else:
        print(f"\n⭐ До следующего уровня: {new_level * 1000 - new_xp} XP")
    ...
    # Переносим длинный текст для удобства чтения
    for i in range(0, len(advice), 60):
        print(f"   {advice[i:i+60]}")
    
    print(f"\n📊 Финансовый скор: {current_score}/100")
    
    if current_score >= 80:
        print("   🏆 Отличный результат! Ты в топ-20% пользователей")
    elif current_score >= 60:
        print("   ✅ Хороший результат. Есть куда расти!")
    else:
        print("   💪 Начало пути. Маленькие шаги ведут к большим результатам!")
    
    print(f"\n🎮 Награда персонажу:")
    print(f"   +{xp_earned} XP")
    print(f"   Итого XP: {new_xp}")
    print(f"   Уровень: {new_level}")
    
    if character_result:
        print(f"\n🖼️  Новый облик персонажа:")
        print(f"   {character_result.get('image_url', 'генерируется...')}")
    
    # Статус API-вызовов
    print(f"\n🔌 Статус интеграции Alem AI:")
    print(f"   AlemLLM:      {'✅ OK' if llm_result['success'] else '⚠️  Демо-режим'}")
    print(f"   Score API:    {'✅ OK' if score_result['success'] else '⚠️  Демо-режим'}")
    if character_result:
        print(f"   Text-to-Image: {'✅ OK' if character_result['success'] else '⚠️  Демо-режим'}")
    
    print("\n" + "=" * 60)
    print("✨ Демонстрация завершена!")
    print("   Для полной работы: установи ALEM_API_KEY и запусти снова.")
    print("=" * 60)
    ...
    return {
        ...
        "new_level": new_level,
        "advice": llm_result.get("advice"),
        "character_image": character_result.get("image_url") if character_result else None
    }


# Точка входа — запускается при python alem_integration_demo.py
if __name__ == "__main__":
    result = run_finquest_demo()
